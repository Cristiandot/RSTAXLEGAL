...
export const metadata = { title: "Carga de documentos — RS Tax & Legal" };
...
export default async function CargaDocumentosPage({
  ...
  const documentos: DocumentoContable[] = (docsRes.data ?? []).map((d) => {
    const u = d.usuarios as unknown as { nombre: string } | { nombre: string }[] | null;
    return {
      id: d.id,
  ...
  const ivaEjecuciones: IvaEjecucionRow[] = (ivaRes.data ?? []).map((e) => {
    const u = e.usuarios as unknown as { nombre: string } | { nombre: string }[] | null;
    return {
      id: e.id,
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 pb-10 sm:px-6">
      <div className="pt-2">
        <ContabilidadTabs clienteId={clienteId} periodo={periodo} active="documentos" />
      </div>
      <div className="mt-4">
