...
- **Des-hardcodeo de plantillas SARGO (✓ 10-06-2026):** los datos variables que estaban escritos fijos en las 13 plantillas pasaron a placeholders (script `scripts/patch-placeholders-sargo.js`, idempotente): (1) asignación de pérdida de caja ("$ 3.000 Tres Mil" / "$XXXXX (en palabras)" / "$3.000 (tres mil pesos)") → `{ASIGNACION_CAJA_TEXTO}`, alimentado por `remuneracion.perdida_caja` (campo nuevo en portal y form interno; 0 → "No se pacta"); (2) lugar de firma "En (la ciudad de) Curauma" → `{CIUDAD_FIRMA}`, alimentado por col nueva `clientes.lugar_firma_contrato` (fallback ciudad → comuna; Sargo = 'Curauma', puede diferir de la comuna tributaria); (3) "chileno" fijo en la comparecencia (incluso versiones extranjero) se eliminó — la nacionalidad ya sale de `{NACIONALIDAD_EMPLEADO}`. **Horas parametrizadas (✓ 10-06-2026):** la cláusula de jornada de los 11 contratos usa `{HORAS_SEMANALES}` (motor: completa → 42, parcial → horas pactadas). Esto corrigió de paso 4 plantillas que decían "44 horas semanales" — tope derogado desde el 26-04-2026 (Ley 21.561: 42 hrs; bajará a 40 el 26-04-2028, ajustar el default del motor entonces). Cuidado al parchear .docx: hay espacios duros (NBSP, 0xA0) entre palabras — usar `[\s ]` en las regex. **Duplicado ATENDEDOR PT pendiente de Felipe:** "ATENDEDOR PT - Plazo Fijo" (redacción vieja, 33 párrafos, registrada horas=null como genérica parcial) vs "ATENDEDOR PT 21h" (redacción moderna, 153 párrafos, horas=21). Mallas de turnos idénticas (mismos hashes), horas ya parametrizadas → para unificar solo falta que Felipe elija la redacción; OJO: la moderna trae cláusulas de "DEPENDIENTE DE ASTOP" (parrillas, coolers) dentro de un contrato rotulado Atendedor de pista — revisar antes de dejarla como única. Al unificar: dejar una con `horas_semanales=null` y desactivar la otra (`activo=false`, soft).
