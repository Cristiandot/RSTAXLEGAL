-- Comprobante de pago del F29 declarado (SII getComprobantePec). Permite ver
-- pagó-total vs postergó/impago y afinar el seguimiento de cobro. 20-07-2026.
-- La grilla /f29 (v_checklist_f29) expone sii_pagado/sii_pago_fecha/sii_pago_monto.
alter table sii_f29_estado add column if not exists pagado boolean;
alter table sii_f29_estado add column if not exists pago_fecha date;
alter table sii_f29_estado add column if not exists pago_monto bigint;
alter table sii_f29_estado add column if not exists pago_banco text;
comment on column sii_f29_estado.pagado is 'true si el SII tiene comprobante de pago del F29 (getComprobantePec). null = aún no consultado; false = declarado sin pago.';
