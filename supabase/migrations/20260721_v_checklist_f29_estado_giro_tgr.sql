-- Estado «Giro TGR» (Cristian 21-07-2026): un F29 declarado con folio pero SIN
-- comprobante de pago en el SII y con monto > 0 = "recibida por internet" → se
-- giró a Tesorería, NO está pagado. Se distingue de «Declarado» (pagado en SII o
-- por RS, o sin monto que pagar). Pago desconocido (null) se mantiene «Declarado».
    ...
    cf.observaciones,
        CASE
            WHEN cf.folio_f29 IS NOT NULL AND btrim(cf.folio_f29) <> ''::text THEN
                CASE
                    WHEN cf.fecha_pago_f29 IS NOT NULL THEN 'Declarado'::text
                    WHEN s.pagado IS TRUE THEN 'Declarado'::text
